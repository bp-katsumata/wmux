---
name: release
description: Step-by-step wmux release process — build, ASAR pack, staging, zip, tag, publish
---
...
# wmux_v_extracted/ dir and avoids picking up stale files from the project root.
...
- **rcedit named export**: `const { rcedit } = require('rcedit')`. Non-destructured form throws "rcedit is not a function".
- **asar `--unpack` glob silently fails on Git Bash for Windows**: use `--unpack-dir node_modules/node-pty/prebuilds` (path-based) instead.
- **Bash cwd drift can recursively pollute staging**: always use subshells `( cd dir && cmd )` or absolute paths.
